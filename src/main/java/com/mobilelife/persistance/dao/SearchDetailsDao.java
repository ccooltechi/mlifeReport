...
public class SearchDetailsDao {
	...
	public int countByCatagory(String catagory, Date fromDt ,Date toDt)
	{
		Session session = HibernateSessionManager.getSession();
		int rid = 0;
		try {
			...
			String query = "Select count(*) from search_details where searchRequest like '%\"category\":\""+catagory+"\"%' and creation_datetime between '"+fromDt+"' and '"+toDt+"'";
			logger.debug("query in countByCatagory in  search_details " + query);
			resultList = new HibernateDAO().findBySQLQuery(session, SearchDetails.class, query,"search_details");

			logger.debug("findID  in  UserInfo Size = "+resultList.size());
			if ((null!=resultList) && (resultList.size()>0))
			{
            	Double ratingx = Double.parseDouble(resultList.get(0)+"");
            	rid = ratingx.intValue();
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
